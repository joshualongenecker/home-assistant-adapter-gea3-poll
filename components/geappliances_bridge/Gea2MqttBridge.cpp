...
static void SavePollingListToNVStore(self_t *self)
{
  ...
  if(nvStorage.begin("storage", RW_MODE)) {
    ...
    snprintf(buffer, sizeof(buffer), "Initial free entries = %d\n", static_cast<int>(freeEntries));
    Serial.print(buffer);
    size_t bytesWritten = nvStorage.putBytes("erdList", self->erd_polling_list, sizeof(self->erd_polling_list));
    snprintf(buffer, sizeof(buffer), "Wrote %d bytes to store list\n", static_cast<int>(bytesWritten));
    Serial.print(buffer);
    bytesWritten = nvStorage.putUInt("erdCount", self->pollingListCount);
    snprintf(buffer, sizeof(buffer), "Wrote %d bytes to store erd count of %d\n", static_cast<int>(bytesWritten), self->pollingListCount);
    Serial.print(buffer);
    bytesWritten = nvStorage.putUChar("erdAddress", self->erd_host_address);
    snprintf(buffer, sizeof(buffer), "Wrote %d bytes to store GEA address of 0x%02X\n", static_cast<int>(bytesWritten), self->erd_host_address);
    Serial.print(buffer);
    freeEntries = nvStorage.freeEntries();
    snprintf(buffer, sizeof(buffer), "Final free entries = %d\n", static_cast<int>(freeEntries));
    Serial.print(buffer);
    nvStorage.end();
  }
}
...
static tiny_hsm_result_t State_IdentifyAppliance(tiny_hsm_t *hsm, tiny_hsm_signal_t signal, const void *data)
{
  ...
  switch(signal) {
    ...
    case signal_timer_expired: {
      snprintf(buffer, sizeof(buffer), "Asking for appliance type ERD 0x0008 from address 0x%02X\n", self->erd_host_address);
      Serial.print(buffer);
    ...
    case signal_read_completed: {
      ...
      if(args->read_completed.erd == 0x0008) {
        ...
        snprintf(buffer, sizeof(buffer), "Using GEA address 0x%02X\n", self->erd_host_address);
        Serial.print(buffer);
      }
...
static void AddErdToPollingList(self_t *self, tiny_erd_t erd)
{
  ...
  char buffer[40];
  snprintf(buffer, sizeof(buffer), "#%d Add ERD erd %04X to polling list\n", self->pollingListCount, erd);
  Serial.print(buffer);
}
